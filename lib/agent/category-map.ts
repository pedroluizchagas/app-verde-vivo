...
const expenseKeywords: Array<{ keywords: string[]; category: string; parent?: string }> = [
  {
    keywords: ["gasolina", "combustível", "combustivel", "diesel", "etanol"],
    category: "Combustível",
    parent: "Operacional",
  },
  {
    keywords: ["óleo 2t", "oleo 2t", "óleo", "oleo", "lubrificante"],
    category: "Lubrificantes",
    parent: "Manutenção de Equipamentos",
  },
  {
    keywords: ["manutenção", "manutencao", "reparo"],
    category: "Manutenção",
    parent: "Manutenção de Equipamentos",
  },
  {
    keywords: ["epi", "epIs", "luvas", "óculos", "oculos", "protetor auricular", "capacete"],
    category: "EPIs",
    parent: "Operacional",
  },
  {
    keywords: ["ferramenta", "ferramentas", "parafusadeira", "serra", "pá", "pa", "enxada"],
...
export function classifyText(text: string): Classification | null {
  ...
  for (const entry of inventoryKeywords) {
    if (entry.keywords.some((k) => t.includes(k))) {
      ...
      return {
        ...
        parent_category_name: entry.parent,
  ...
  if (["gasolina", "combustível", "combustivel", "diesel", "etanol"].some((k) => t.includes(k))) {
    ...
    if (machinesHints.some((k) => t.includes(k))) {
      return {
        kind: "expense",
        category_name: "Combustível Máquinas",
        parent_category_name: "Operacional",
      };
    }
    if (vehicleHints.some((k) => t.includes(k))) {
      return {
        kind: "expense",
        category_name: "Combustível Veículos",
        parent_category_name: "Operacional",
      };
    }
