...
export async function runAssistant(
  ...
  if (parsed.intent === "record_expense") {
    ...
    if (
      (!providedTrx ||
        (providedTrxDate &&
          providedTrxDate > now &&
          /\bontem|semana passada|passad[ao]\b/i.test(input))) &&
      resolvedTrx
    ) {
  ...
  if (!validation.ok) {
    const needList = validation.need?.length
      ? `Campos faltantes: ${validation.need.join(", ")}.`
      : "Dados insuficientes.";
    return {
      reply: `${parsed.reply || "Preciso de mais detalhes."} ${needList}`,
      intent: parsed.intent,
      result: { ok: false, need: validation.need },
...
async function getUserPreferences(
  userId: string,
  token?: string,
): Promise<{ credit_card_due_day?: number | null; default_pending_days?: number | null } | null> {
  const supabase = token ? createClientWithToken(token) : await createSupabaseServer();
...
function resolveNaturalDate(text: string): string | undefined {
  ...
  const hasThis =
    /\b(esta|nesta)\b/.test(t) || /\besta semana\b/.test(t) || /\bnesta semana\b/.test(t);
