...
export function MovementForm({
  ...
  const isAreaUnit = useMemo(() => {
    const raw = String(selectedProduct?.unit || "")
      .toLowerCase()
      .trim();
  ...
  const computedCalcQty = useMemo(() => {
    const area = Number(lengthM) * Number(widthM) * Math.max(Number(areasCount), 1);
  ...
  const totalValue = useMemo(() => Number(quantity) * effectiveCost, [quantity, effectiveCost]);
          ...
          <CardContent className="p-5 flex flex-col gap-5">
                  ...
                  {products.length > 0 ? (
                    products.map((p) => (
                      <SelectItem key={p.id} value={p.id}>
                        {p.name}
                        <span className="text-muted-foreground ml-1.5">({p.unit})</span>
                      </SelectItem>
                    ))
                  ) : (
            ...
            <div className="border-t border-border/60 pt-4 flex flex-col gap-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="movement_date" className="text-[12px] font-medium">
                    Data
                  </Label>
                  <Input
                    id="movement_date"
            ...
            {isIn && (
              <div className="border-t border-border/60 pt-4 flex flex-col gap-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="flex flex-col gap-1.5">
                    <Label htmlFor="unit_cost" className="text-[12px] font-medium">
                      Custo unitário (R$)
                    </Label>
            ...
            <div className="border-t border-border/60 pt-4 flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="appointment" className="text-[12px] font-medium">
                  Vincular a serviço{" "}
      ...
      <AlertDialog open={openCalc} onOpenChange={setOpenCalc}>
        <AlertDialogContent>
          ...
          <div className="grid grid-cols-2 gap-3 py-2">
            <div className="flex flex-col gap-1.5">
              <Label className="text-[12px] font-medium">Comprimento (m)</Label>
              <Input
                type="number"
                step="0.01"
                min="0"
            ...
            <div className="flex flex-col gap-1.5">
              <Label className="text-[12px] font-medium">Nº de áreas</Label>
              <Input
                type="number"
                min="1"
                value={areasCount}
          ...
          <div className="rounded-lg bg-muted px-4 py-3 flex items-center justify-between">
            <span className="text-[12px] text-muted-foreground">Quantidade calculada</span>
            <span className="text-[16px] font-bold tabular-nums">{fmtQty(computedCalcQty)} m²</span>
          </div>
          <AlertDialogFooter>
