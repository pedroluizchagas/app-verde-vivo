...
export function WorkOrderServiceNoteRich({
  ...
  const dateStr = (() => {
    const d = order?.appointment?.scheduled_date
      ? new Date(order.appointment.scheduled_date)
      : new Date(order?.created_at || NOW_MS);
  ...
  const buildImage = async () => {
    ...
    ctx.fillText("Serviço de jardinagem", margin + colW + colGap, headerH + 52);
    ctx.strokeStyle = "#e9e9e9";
    ctx.beginPath();
    ...
    ctx.strokeStyle = "#e9e9e9";
    ctx.beginPath();
    ctx.moveTo(margin, headerH + 156);
    ctx.lineTo(bw - margin, headerH + 156);
    ctx.stroke();
    let y = headerH + 182;
    ctx.fillStyle = "#666";
    ctx.font = "500 13px system-ui";
    ctx.fillText("Itens", margin, y);
    y += 22;
    ctx.font = "400 13px system-ui";
    ctx.fillStyle = "#111";
    const list =
    ...
    if (singleLineFits) {
      ...
      for (const ln of companyWrapped) {
        ctx.fillText(ln, margin, y);
        y += 16;
      }
  ...
  const handleExportPdf = async () => {
    ...
    reader.onload = () => {
      ...
      doc?.open();
      doc?.write(
        `<!doctype html><html><head><meta charset='utf-8'><title>nota-os-${order?.id}</title><style>${style}</style></head><body><img src='${dataUrl}'/></body></html>`,
      );
      doc?.close();
                ...
                items.map((it) => (
                  <div key={it.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {Number(it.quantity)} {String(it.unit || it.product?.unit || "un")} •{" "}
