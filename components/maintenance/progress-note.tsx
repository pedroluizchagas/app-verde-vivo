...
export function MaintenanceProgressNote({
  ...
  const data = useMemo(() => {
    const periodExecs = (executions || []).filter(
      (e) => new Date(String(e.created_at)) >= start && String(e.status) === "done",
    );
    let totalExec = 0;
    let totalFert = 0;
    let totalPests = 0;
    ...
    const checklistSummary = Object.keys(checklistAgg).map((k) => ({
      label: k,
      pct:
        checklistAgg[k].total > 0
          ? Math.round((checklistAgg[k].done / checklistAgg[k].total) * 100)
          : 0,
    }));
  ...
  const buildImage = async () => {
    ...
    for (const it of list) {
      ...
      ctx.fillStyle = "#666";
      ctx.fillText(pct, bw - margin - wRight, y);
      ctx.fillStyle = "#111";
      y += 24;
    }
    ctx.strokeStyle = "#e9e9e9";
