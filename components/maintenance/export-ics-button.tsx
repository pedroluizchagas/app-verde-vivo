...
export function ExportMaintenanceICSButton({ planId }: { planId: string }) {
    ...
    try {
      ...
      if (exec?.appointment_id) {
        ...
        if (ap) {
          ...
          const e = ap.end_date
            ? new Date(String(ap.end_date))
            : new Date(s.getTime() + Number(ap.duration_minutes || 60) * 60000);
          start = s;
          end = e;
        }
      }
  ...
  return (
    <div className="flex items-center gap-2">
      <Button variant="outline" className="bg-transparent" disabled={loading} onClick={downloadICS}>
        <Calendar className="h-4 w-4 mr-2" /> Exportar .ics
      </Button>
