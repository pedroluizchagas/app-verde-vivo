...
export function QuickActionForm({ planId }: { planId: string }) {
  ...
  const [checklist, setChecklist] = useState<
    { key: string; label: string; done: boolean; notes?: string }[]
  >([
  ...
  const [fertilization, setFertilization] = useState<
    { product: string; dose: string; area: string; date?: string }[]
  >([]);
  const [pests, setPests] = useState<
    { type: string; severity: string; treatment: string; date?: string; notes?: string }[]
  >([]);
  ...
  const totals = useMemo(() => {
    ...
    const matsRaw = materials.reduce(
  ...
  const buildNoteText = () => {
    const lines = [
      ...
      ...(checklist.length > 0
        ? checklist.map(
            (c) => `${c.label} — ${c.done ? "feito" : "pendente"}${c.notes ? ` (${c.notes})` : ""}`,
          )
        : ["Nenhum"]),
      `Adubação:`,
      ...
      ...(materials.length > 0
        ? materials.map(
            (m) =>
              `${m.name || "Material"} — ${Number(m.quantity || 0)} ${m.unit || "un"} (${currency(Number(m.quantity || 0) * Number(m.unitCost || 0))})`,
          )
        : ["Nenhum"]),
      ...
      planDefaults?.client
        ? `Cliente: ${planDefaults.client.name} | ${planDefaults.client.phone || ""} | ${planDefaults.client.address || ""}`
        : undefined,
    ].filter(Boolean);
    return lines.join("\n");
  };
  ...
  const addMaterial = () =>
    setMaterials((list) => [...list, { name: "", quantity: "", unit: "", unitCost: "" }]);
  const updateMaterial = (idx: number, patch: Partial<MaterialEntry>) =>
    setMaterials((list) => list.map((m, i) => (i === idx ? { ...m, ...patch } : m)));
  const removeMaterial = (idx: number) => setMaterials((list) => list.filter((_, i) => i !== idx));
  const toggleChecklist = (key: string, done: boolean) =>
    setChecklist((list) => list.map((c) => (c.key === key ? { ...c, done } : c)));
  const setChecklistNotes = (key: string, notes: string) =>
    setChecklist((list) => list.map((c) => (c.key === key ? { ...c, notes } : c)));
  const addFertilization = () =>
    setFertilization((list) => [...list, { product: "", dose: "", area: "", date: "" }]);
  const updateFertilization = (
    idx: number,
    patch: Partial<{ product: string; dose: string; area: string; date?: string }>,
  ) => setFertilization((list) => list.map((f, i) => (i === idx ? { ...f, ...patch } : f)));
  const removeFertilization = (idx: number) =>
    setFertilization((list) => list.filter((_, i) => i !== idx));
  const addPest = () =>
    setPests((list) => [...list, { type: "", severity: "", treatment: "", date: "", notes: "" }]);
  const updatePest = (
    idx: number,
    patch: Partial<{
      type: string;
  ...
  ) => setPests((list) => list.map((p, i) => (i === idx ? { ...p, ...patch } : p)));
  ...
  const updatePhoto = (idx: number, url: string) =>
    setPhotos((list) => list.map((u, i) => (i === idx ? url : u)));
    ...
    <form onSubmit={handleSubmit} className="grid gap-3">
          ...
          {checklist.map((c) => (
            <div key={c.key} className="grid grid-cols-4 gap-2 items-center">
              ...
              <div className="col-span-3">
                <Input
                  value={c.notes || ""}
                  onChange={(e) => setChecklistNotes(c.key, e.target.value)}
        ...
        <div className="space-y-2">
          {fertilization.map((f, idx) => (
            <div key={idx} className="grid grid-cols-5 gap-1">
              <Input
                value={f.product}
                onChange={(e) => updateFertilization(idx, { product: e.target.value })}
                placeholder="Produto"
              />
              <Input
                value={f.dose}
                onChange={(e) => updateFertilization(idx, { dose: e.target.value })}
                placeholder="Dose"
              />
              <Input
                value={f.area}
                onChange={(e) => updateFertilization(idx, { area: e.target.value })}
                placeholder="Área"
              />
              <Input
                type="date"
                value={f.date || ""}
                onChange={(e) => updateFertilization(idx, { date: e.target.value })}
              />
              <Button
                type="button"
                variant="outline"
                className="bg-transparent"
                onClick={() => removeFertilization(idx)}
              >
                Remover
              </Button>
            </div>
        ...
        <div className="space-y-2">
          {pests.map((p, idx) => (
            <div key={idx} className="grid grid-cols-6 gap-1">
              <Input
                value={p.type}
                onChange={(e) => updatePest(idx, { type: e.target.value })}
                placeholder="Tipo"
              />
              <Input
                value={p.severity}
                onChange={(e) => updatePest(idx, { severity: e.target.value })}
                placeholder="Severidade"
              />
              <Input
                value={p.treatment}
                onChange={(e) => updatePest(idx, { treatment: e.target.value })}
                placeholder="Tratamento"
              />
              <Input
                type="date"
                value={p.date || ""}
                onChange={(e) => updatePest(idx, { date: e.target.value })}
              />
              <Input
                value={p.notes || ""}
                onChange={(e) => updatePest(idx, { notes: e.target.value })}
          ...
          <Button type="button" variant="outline" className="bg-transparent" onClick={addPest}>
            Adicionar praga
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-2">
        ...
        <div className="space-y-2">
          ...
          <Button type="button" variant="outline" className="bg-transparent" onClick={addPhoto}>
            Adicionar foto
          </Button>
        </div>
      </div>
