...
export function GenerateMaintenanceCertificateButton({ planId }: { planId: string }) {
  ...
  const shareLatest = async () => {
    ...
    try {
      ...
      const canvasBlob = await (async () => {
        ...
        const total = Number(
          (typeof latest.final_amount === "number"
            ? latest.final_amount
            : labor + materialsTotal
          ).toFixed(2),
        );
        const dateStr = new Date(String(latest.created_at || Date.now())).toLocaleString("pt-BR", {
          day: "2-digit",
          month: "short",
          year: "numeric",
          hour: "2-digit",
        ...
        ctx.fillStyle = "#fff";
        ctx.fillRect(0, 0, bw, 600);
        ctx.fillStyle = "#111";
        ctx.font = "700 26px system-ui";
        ctx.fillText("Comprovante de manutenção", margin, 48);
        ...
        ctx.lineTo(bw - margin, 150);
        ctx.stroke();
        ctx.fillStyle = "#666";
        ctx.font = "500 13px system-ui";
        ctx.fillText("Valores", margin, 174);
        ctx.fillStyle = "#111";
  ...
  return (
    <div className="flex items-center justify-center">
      <Button className="gap-2" disabled={loading} onClick={shareLatest}>
        <ImageIcon className="h-4 w-4" /> Gerar Comprovante
      </Button>
