...
export function MaintenanceServiceNoteRich({
  ...
  const materialsTotal = Number(
    (materialsTotalRaw * (1 + (markupPct > 0 ? markupPct / 100 : 0))).toFixed(2),
  );
  const total = Number(
    (typeof execution.final_amount === "number"
      ? execution.final_amount
  ...
  const buildImage = async () => {
    ...
    const bh =
      ...
      photosH +
      54 +
      clientNameLines.length * 16 +
      addressLines.length * 16 +
      footerH;
    ...
    await new Promise((res) => {
      logo.onload = () => res(null);
      logo.onerror = () => res(null);
    });
    try {
      ctx.drawImage(logo, bw - margin - 36, margin, 36, 36);
    } catch {}
    ctx.strokeStyle = "#e9e9e9";
    ctx.beginPath();
    ctx.moveTo(margin, headerH);
    ctx.lineTo(bw - margin, headerH);
    ctx.stroke();
    ...
    ctx.strokeStyle = "#e9e9e9";
    ctx.beginPath();
    ctx.moveTo(margin, headerH + 72);
    ctx.lineTo(bw - margin, headerH + 72);
    ctx.stroke();

    let y = headerH + 92;
    ctx.fillStyle = "#666";
    ctx.font = "500 13px system-ui";
    ctx.fillText("Materiais", margin, y);
    y += 22;
    ...
    for (const it of list) {
      ...
      ctx.fillStyle = "#666";
      ctx.fillText(right, bw - margin - wRight, y);
      ctx.fillStyle = "#111";
      y += 28;
    }
    ctx.strokeStyle = "#e9e9e9";
    ...
    ctx.beginPath();
    ctx.moveTo(margin, y);
    ctx.lineTo(bw - margin, y);
    ctx.stroke();
    y += 24;

    ctx.fillStyle = "#666";
    ctx.font = "500 13px system-ui";
    ctx.fillText("Checklist", margin, y);
    y += 22;
    ctx.fillStyle = "#111";
    ctx.font = "400 13px system-ui";
    ...
    ctx.fillStyle = "#666";
    ctx.font = "500 13px system-ui";
    ctx.fillText("Adubação", margin, y);
    y += 22;
    ctx.fillStyle = "#111";
    ctx.font = "400 13px system-ui";
    ...
    ctx.fillStyle = "#111";
    if (client?.name) {
      ctx.fillText(String(client.name), margin, y);
      y += 16;
    }
    if (client?.phone) {
      ctx.fillText(String(client.phone), margin, y);
      y += 16;
    }
    if (client?.address) {
  ...
  const buildNoteText = () => {
    const lines = [
      ...
      client
        ? `Cliente: ${client.name} | ${client.phone || ""} | ${client.address || ""}`
        : undefined,
    ].filter(Boolean);
    return lines.join("\n");
  };
    ...
    <Card>
      ...
      <CardContent>
        <div id={selectorId} className="rounded-md border p-4 space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Status: {String(execution.status || "done")}
            </p>
          </div>
          {description && (
            <div>
              ...
              {pests.length > 0 ? (
                pests.map((p: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {String(p.severity || "")} • {String(p.treatment || "")}
                    </span>
                  </div>
                ))
              ) : (
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Button variant="outline" className="gap-2" onClick={handleShareImage}>
          ...
          <Button variant="outline" className="gap-2" onClick={handleShareText}>
            <Share2 className="h-4 w-4" /> Enviar texto
          </Button>
        </div>
      </CardContent>
    </Card>
