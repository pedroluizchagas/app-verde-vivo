      ...
      <CardContent>
                ...
                materials.map((m) => (
                  <div key={m.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {currency(
                        Number(m.quantity) *
                          Number(m.unit_cost ?? m.product?.cost ?? 0) *
                          (1 + (marginPct > 0 ? marginPct / 100 : 0)),
                      )}
                    </span>
                  </div>
                ))
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <ExportDashboardPDFButton
            selector={`#${selectorId}`}
            fileName={`nota-servico-${appointment.id}`}
          />
        </div>
      </CardContent>
    </Card>
...
function buildNoteText(
  ...
  const lines = [
    ...
    appointment.client
      ? `Cliente: ${appointment.client.name} | ${appointment.client.phone} | ${appointment.client.address}`
      : undefined,
  ].filter(Boolean);
  return lines.join("\n");
}
