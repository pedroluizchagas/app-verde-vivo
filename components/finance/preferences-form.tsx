...
export function PreferencesForm({
  ...
  initial?: {
    ...
    default_pending_days?: number | null;
    default_product_margin_pct?: number | null;
  };
}) {
  ...
  const [productMargin, setProductMargin] = useState<number>(
    initial?.default_product_margin_pct ?? 0,
  );
  ...
  useEffect(() => {
    setMessage(null);
  }, [creditDue, pendingDays, productMargin]);
      ...
      <CardContent className="grid gap-3 p-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Usado para despesas no crédito (definimos o vencimento automaticamente).
          </p>
        </div>
        <div>
          ...
          />
          <p className="text-xs text-muted-foreground mt-1">
            Usado para despesas pendentes quando não há outro vencimento indicado.
          </p>
        </div>
        <div>
          ...
          <Input
            ...
            step={0.1}
            value={productMargin}
            onChange={(e) => setProductMargin(Number(e.target.value))}
            placeholder="Ex.: 20"
          />
          <p className="text-xs text-muted-foreground mt-1">
            Aplicada automaticamente ao preço dos materiais em serviços e orçamentos. Não é exibida
            ao cliente como margem separada.
          </p>
        </div>
