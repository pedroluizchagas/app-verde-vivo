...
export async function POST(req: Request) {
  try {
    ...
    if (!planId)
      return NextResponse.json({ ok: false, error: "planId é obrigatório" }, { status: 400 });
    if (!date)
    ...
    if (!allDay && start >= end)
      return NextResponse.json(
        { ok: false, error: "Horário término deve ser após início" },
        { status: 400 },
      );
    ...
    if (hasConflict)
      return NextResponse.json(
        { ok: false, error: "Conflito de horário na agenda" },
        { status: 409 },
      );
    ...
    if (exec?.appointment_id)
      return NextResponse.json({ ok: true, existed: true, id: exec.appointment_id });
    ...
    if (exec?.id) {
      ...
      await supabase
        .from("plan_executions")
        .insert([{ plan_id: planId, cycle, appointment_id: appt?.id, status: "open" }]);
    }
