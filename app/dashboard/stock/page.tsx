    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-1">Produtos e movimentacoes</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Card className="py-0">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] text-muted-foreground font-medium">Produtos</span>
              <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center shrink-0">
                <Package className="h-3.5 w-3.5 text-muted-foreground" />
              </div>
            </div>
            <p className="text-[22px] font-bold leading-tight mb-0.5">{totalProducts}</p>
        ...
        <Card className="py-0">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] text-muted-foreground font-medium">Estoque Baixo</span>
              <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center shrink-0">
            ...
            <p className="text-[10px] text-muted-foreground">Abaixo do minimo</p>
          </CardContent>
        </Card>
        ...
        <Card className="py-0">
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-[11px] text-muted-foreground font-medium">Movimentacoes</span>
              <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-3">
        <Card className="py-0">
          <CardContent className="p-4">
            <h2 className="text-[14px] font-semibold mb-3">Produtos e saldo atual</h2>
            {products && products.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className={`text-sm font-bold ${isLow ? "text-red-400" : ""}`}>
                          {number(stock)} {p.unit}
                        </p>
                        <p className="text-[10px] text-muted-foreground">
              ...
              <p className="text-[12px] text-muted-foreground">Nenhum produto cadastrado.</p>
            )}
          </CardContent>
        </Card>
            ...
            {lastIn ? (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[12px] font-medium">{lastInProduct?.name}</p>
                    <p className="text-[10px] text-muted-foreground">
                      {number(Number(lastIn.quantity))} {lastInProduct?.unit} -- Unit:{" "}
                      {currency(Number(lastIn.unit_cost))}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold">{currency(lastTotal)}</p>
                    <p className="text-[10px] text-muted-foreground">
                      {new Date(lastIn.movement_date).toLocaleDateString("pt-BR")}
                    </p>
                  </div>
                </div>
                <p className="text-[10px] text-muted-foreground">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  {lastExpense && (
                    <Button asChild variant="outline" size="sm">
                      <Link href={`/dashboard/finance/transactions/${lastExpense.id}`}>
                        Lancamento
                      </Link>
                    </Button>
                  )}
              ...
              <p className="text-[12px] text-muted-foreground">Sem compras registradas.</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <Tabs defaultValue="recent">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-[12px] font-medium">
                          {m.product?.name} ({number(Number(m.quantity))} {m.product?.unit})
                        </p>
                        <p className="text-[10px] text-muted-foreground">
          ...
          <TabsContent value="outs" className="mt-3">
            ...
            recentMovementsNorm.filter((m) => m.type === "out").length > 0 ? (
              ...
              <p className="text-[12px] text-muted-foreground py-6 text-center">Sem saidas.</p>
            )}
          </TabsContent>
          <TabsContent value="ins" className="mt-3">
            ...
            recentMovementsNorm.filter((m) => m.type === "in").length > 0 ? (
              ...
              <p className="text-[12px] text-muted-foreground py-6 text-center">Sem entradas.</p>
            )}
          </TabsContent>
        </Tabs>
      </div>
