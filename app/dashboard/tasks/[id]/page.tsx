  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[13px] text-muted-foreground capitalize">{createdStr}</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {dueStr && (
          <div
            className={`flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full bg-muted ${dueDateColor}`}
          >
            {isOverdue ? (
      ...
      {task.description && (
        <Card className={`py-0 border-l-4 ${borderColor}`}>
          <CardContent className="p-5">
            ...
            <p className="text-[14px] leading-relaxed whitespace-pre-wrap">{task.description}</p>
          </CardContent>
        </Card>
      )}

      {/* Descrição organizada pela IA */}
      {task.organized_description && task.organized_description !== task.description && (
      ...
      {!task.description && !task.organized_description && (
        <Card className={`py-0 border-l-4 ${borderColor}`}>
          <CardContent className="p-5">
            ...
            <Button asChild variant="outline" size="sm" className="h-8 rounded-lg text-[12px] mt-3">
              <Link href={`/dashboard/tasks/${id}/edit`}>Adicionar descrição</Link>
            </Button>
          </CardContent>
        </Card>
      )}
